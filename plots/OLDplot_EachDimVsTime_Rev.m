...
for iComponent = 1:nComponents
    ...
    try
        title(par.titles{iComponent}, 'Interpreter', 'latex', 'FontSize', 16);
    catch
    end
...
if isfield(par,'legendLocation') && ~isempty(cc)
    ...
    if strcmpi(par.legendLocation,'right')
        % riduci la larghezza di tutti i subplot per lasciare spazio alla legenda
        ax = findobj(hfig,'Type','Axes');
        ax = ax(~strcmp(get(ax,'Tag'),'legend'));   % esclude eventuale legenda
        
        for iax = 1:numel(ax)
            posAx = get(ax(iax),'Position');
            posAx(3) = posAx(3) * 0.82;   % riduce larghezza
    ...
    elseif strcmpi(par.legendLocation,'bottom')
        % alza un po' i subplot per fare spazio sotto
        ax = findobj(hfig,'Type','Axes');
