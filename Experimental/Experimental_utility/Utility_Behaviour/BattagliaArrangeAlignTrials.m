function   [data_trials, session_data, Trials]= BattagliaArrangeAlignTrials(par)
% function [data_trials, session_data, Trials]= BattagliaArrangeAlignTrials(par)
% 
...
session_data    = lfpSessionExtractAlign(session_name,dmode,selS,selK,tstart,tstop,AlignEvent);
InField         = par.InField;
xlf             = 'time';
...
for iTrial=1:nTrials
    ...
    data_trials(iTrial).([xlf InField])       =session_data.Trials(iTrial).RawTime;
    data_trials(iTrial).(InField)           =session_data.LFP{iTrial}';
    data_trials(iTrial).JXYEXY              =session_data.Trials(iTrial).JXYEXY;
    data_trials(iTrial).([xlf 'JXYEXY'])    =session_data.Trials(iTrial).timeJXYEXY;
    ...
    % data_trials(iTrial).trialName   =Labels{data_trials(iTrial).trialType};g
