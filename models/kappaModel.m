function k_cohen = kappaModel(x,varargin)
...
switch w
    ...
    function kcomp
        ...
        k_cohen=(po-pe)/(1-pe); %Cohen's kappa
        ...
        ratio=k_cohen/km; %observed as proportion of maximum possible
        sek=sqrt((po*(1-po))/(n*(1-pe)^2)); %kappa standard error for confidence interval
        ci=k_cohen+([-1 1].*(abs(-realsqrt(2)*erfcinv(alpha))*sek)); %k confidence interval
        z=k_cohen/sek; %normalized kappa
        ...
        if k_cohen<0
            disp('Poor agreement')
        elseif k_cohen>=0 && k_cohen<=0.2
            disp('Slight agreement')
        elseif k_cohen>=0.21 && k_cohen<=0.4
            disp('Fair agreement')
        elseif k_cohen>=0.41 && k_cohen<=0.6
            disp('Moderate agreement')
        elseif k_cohen>=0.61 && k_cohen<=0.8
            disp('Substantial agreement')
        elseif k_cohen>=0.81 && k_cohen<=1
