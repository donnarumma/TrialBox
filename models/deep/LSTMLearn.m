function   net = LSTMLearn(data_trials,par)
% function net = LSTMLearn(data_trials,par)
InField             = par.InField;
...
X3d                     = permute(X3d,[2,1,3]);                             % nTimes x nCells x nTrials

% X4d                 = reshape(X3d,size(X3d,1),size(X3d,2),1,size(X3d,3)); % nTrials x nChannels x 1 x nTimes
[nTimes, nCells, nTrials] = size(X3d);

XTrain              = X3d(:,:,log_train);
