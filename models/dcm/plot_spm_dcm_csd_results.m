function plot_spm_dcm_csd_results(DCM,Action,fig)
...
Hz  = xY.Hz;                    % PST
co  = lines(nt);                % trial color
cos = summer(nt);               % channel color
ult = {'-', ':', '-.'};
lt  = repmat(ult,1,ceil(nt/length(ult)));
lt  = lt(1:nt);
name = DCM.xY.name;
% switch
%--------------------------------------------------------------------------
...
case{lower('spectral data')}
    ...
    for i = 1:nm
        ...
        subplot(2,2,3)
        for k = 1:nt
            plot(Hz,abs(xY.y{k}(:,i,i)),'color',cos(i,:),'LineStyle',lt{k}), hold on
            axis square, spm_axis tight
        end
    end
...
nu  = length(DCM.M.pE.B);          % Nr experimental inputs % DONNARUMMA
ns  = size(DCM.M.pE.A{1},2);  % Nr of sources  % DONNARUMMA
...
case{lower('Coupling (A)')}
    ...
    for  i = 1:m
        ...
        set(gca,'YTick',1:ns,'YTickLabel',name,'FontSize',8)
        set(gca,'XTick',[])
...
case{lower('trial-specific effects')}
    %% DONNARUMMA
    for iT=1:size(DCM.xU.X,1)
        inds(iT)=find(DCM.xU.X(iT,:)); 
    end 
    cmaps=lines(nu);
    %% end DONNARUMMA
    ...
    for i = 1:ns
        for j = 1:ns
            ...
            for k = 1:nu
                % q = q | DCM.B{k}(i,j);
                q = q | DCM.M.pC.B{k}(i,j); % DONNARUMMA
            end
            ...
            if q
                ...
                hb = gobjects(nu,1);
                for iT=1:length(B)
                    b=bar(iT,exp(B(iT))*100,'facecolor',cmaps(inds(iT),:));
                    hb(inds(iT))=b;
                end
                legend(hb,DCM.xU.name);
                % bar(exp(B)*100,'c')
                %% end DONNARUMMA
                title([name{j}, ' to ' name{i}],'FontSize',10)
...
case{lower('Cross-spectra (sources)')}
    ...
    for i = 1:nm
        ...
        subplot(2,2,3)
        for k = 1:nt
            plot(Hz,abs(DCM.Hs{k}(:,i,i)),'color',cos(i,:)), hold on
            axis square, spm_axis tight
...
case{lower('Cross-spectra (channels)')}
    ...
    for i = 1:nm
        ...
        subplot(2,2,3)
        for k = 1:nt
            plot(Hz,abs(DCM.Hc{k}(:,i,i)),'color',cos(i,:)), hold on
            plot(Hz,abs(DCM.Hc{k}(:,i,i) + DCM.Rc{k}(:,i,i)),':','color',cos(i,:))
            axis square, spm_axis tight
        end
    end
...
case{lower('Covariance (sources)')}
    ...
    for i = 1:nm
        ...
        subplot(2,2,3)
        for k = 1:nt
            plot(pst,ccf{k}(:,i,i),'color',cos(i,:)), hold on
            axis tight square, set(gca,'XLim',[-128 128],'YLim',[p q])
        end
    end
