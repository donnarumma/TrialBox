function   out=lfpSessionExtractMIRCO(session_name,dmode,selS,selK)
% function out=lfpSessionExtract(session_name,dmode,selS,selK)
% example:
% session_name = 'SK009';
try
    dmode;
...
[~,selS] = max(Depth(iLFP_S));
[~,selK] = max(Depth(iLFP_K));
%fprintf('iLFP K\n'); disp(iLFP_K);
try
