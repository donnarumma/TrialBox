...
for indsub=1
    ...
    for iTr=1:length(EEG_trials)
        EEG_app(iTr).data = EEG_trials(iTr).(signal_name);
    end
    for iTr=1:length(EEG_trials)
        EEG_app.data = EEG_trials(iTr).(signal_name);
        % for nCh = 1:size(EEG_app,1)
            EEG_app.data= pop_clean_rawdata(EEG_app.data, 'FlatlineCriterion',5,'ChannelCriterion',0.8,'LineNoiseCriterion',4, ...
                'Highpass','off','BurstCriterion',20,'WindowCriterion','off','BurstRejection','off','Distance','Euclidian');
            EEG_trials(iTr).(signal_name) = EEG_app.data;
        % end
    end
    ...
    par.TimeSelect.t2            = 3.5; % in s from ZeroEvent time
    ...
    par.FilterBankCompute.f_min      = 0.5;
    par.FilterBankCompute.f_max      = 45.5;
...
% time = pVals.timecomparisons;
% for i=1:size(pVals.comparisons,1)
%     [pmin(i),indmin(i)] = min(smooth(pVals.comparisons(i,3:end-2)));
%     disp(pmin(i));
%     disp(time(indmin(i)+2));
% end
% 
% indmin = nan(size(pVals.comparisons,1),1);
% pmin = nan(size(pVals.comparisons,1),1);
% time = pVals.timecomparisons;
% for i=1:size(pVals.comparisons,1)
%     [pmin(i),indmin(i)] = min(smooth(pVals.comparisons(i,:)));
%     disp(pmin(i));
%     disp(time(indmin(i)));
% end